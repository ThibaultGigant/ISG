...
public class MonitorTextSystem : FSystem {
	...
	public MonitorTextSystem(){
		foreach (GameObject go in monitors) {
			...
			m.groundSpeedQueue = new LimitedQueue<Vector3> (m.fps);
	...
	protected override void onProcess(int familiesUpdateCount) {
		...
		foreach (GameObject go in monitors) {
			...
			m.groundSpeedQueue.Enqueue (m.target.transform.position);
			...
			if (m.frameCount == m.fps) {
				...
				float angle = Vector3.Angle (m.groundSpeedQueue.Peek (), m.target.transform.position);
				double groundSpeed = angle * 63710f * 3.6f * Math.PI / (Time.fixedDeltaTime * m.fps * 180f);
